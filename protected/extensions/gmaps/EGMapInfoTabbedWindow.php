<?php
class EGMapInfoTabbedWindow extends EGMapInfoWindow
{
  	/**
   	* 
   	* Config options
  	...
  	*/
  	public function toJs($map_js_name = 'map')
  	{
    	...
    	$return = '';
    	$return .= $this->getJsName().' = new google.maps.InfoWindowTabs('.EGMap::encode($this->options).');'.PHP_EOL;
  	...
  	public function setCustomProperties($custom_properties)
  	{
  		if(!is_array($custom_properties))
  			throw new CException(Yii::t('EGMap','EGMapInfoTabbedWindow custom properties must of type array to be set'));
